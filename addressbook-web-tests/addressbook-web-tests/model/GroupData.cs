        ...
        public List<UserData> GetContacts()
        {
            using (AddressBookDB db = new AddressBookDB())
            {
                return (from c in db.Users
                        from gcr in db.GCR.Where(p=>p.GroupId == this.Id && p.ContactId == c.Id && c.Deprecated == "0000-00-00 00:00:00") select c).Distinct().ToList();
                //db.Close(); //используя using метод выполняется автоматически
            }
        }

        public List<UserData> GetContactsInGroup()
        {
            using (AddressBookDB db = new AddressBookDB())
            {
                return (from c in db.Users
